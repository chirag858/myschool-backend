...
export async function seedDemo() {
  ...
  // The demo `driver` LOGIN owns this bus — the link the mobile driver app
  // scopes everything by. Without it the driver app has no bus to show.
  const driverUser = await UserModel.findOne({ schoolId: school._id, username: 'driver' }).lean();
  const vehicle = await VehicleModel.findOneAndUpdate(
    { schoolId: school._id, registrationNumber: 'PB-11-AB-1234' },
    {
      schoolId: school._id,
      registrationNumber: 'PB-11-AB-1234',
      vehicleType: 'bus',
      seatingCapacity: 40,
      status: 'active',
      driverName: 'Rajesh Singh',
      driverMobile: '9876500001',
      driverUserId: driverUser ? String(driverUser._id) : undefined,
    },
    { upsert: true, new: true, setDefaultsOnInsert: true },
  );
  ...
  // Prefer a Class 1-A rider: those are the demo PARENT's children (see the
  // parent-portal block below), so the demo parent actually has a bus to track.
  const transStudent = allStudents.find((s) => s.className === 'Class 1' && s.section === 'A') ?? allStudents[0];
  if (transStudent) {
  ...
  // Class 1-A FIRST: that is the demo teacher's incharge class, so the demo
  // parent and the demo teacher share a child. Without this the parent's kids
  // were simply the first two students on the roster (Nursery), and every
  // class-scoped teacher↔parent surface (meet links, class circulars,
  // attendance the teacher marks) looked empty on both sides in a demo.
  const inchargeKids = allStudents.filter((s) => s.className === 'Class 1' && s.section === 'A');
  const parentKids = [...inchargeKids, ...allStudents.filter((s) => !inchargeKids.includes(s))].slice(0, 2);
