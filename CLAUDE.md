...
| `src/lib/` | Shared helpers: `api-error.ts`, `api-response.ts`, `async-handler.ts`, `jwt.ts`, `logger.ts`, `paginate.ts`, `messaging-provider.ts` (still-fake bulk SMS/WhatsApp), `sms-provider.ts` (real Fast2SMS — OTP route only, gated on `FAST2SMS_API_KEY`), `mailer.ts` (real SMTP — optional, gated on `SMTP_*` env), `storage.ts` (Cloudflare R2). |
